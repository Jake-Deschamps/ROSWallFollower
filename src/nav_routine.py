...
def nav_logic(my_scan):
    ...
    # Settings
    k_p_x = 5#10       # Proportional gain for distance error -> set angle
    k_p_t = 1#.5    # Proportional gain for theta error -> set angular
    v = 0.13  # 0.15        # Forward velocity for general use
    ...
    theta_error = theta_set - 0.5*(rhs - wall_index)*np.pi/180
    w = k_p_t * theta_error
    ...
    nav_logic.set_angular = w
    rospy.logdebug(wall_str + " " + f"setting angular velocity {w:.3f}")
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        if client.get_state() < 2:
            ...
            rospy.logdebug(
                f"Publishing lin speed {set_twist.linear.x:.3f}, angular speed {set_twist.angular.z:.3f}")
